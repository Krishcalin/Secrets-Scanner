...
from core.reporter import result_to_dict, write_report
...
def _emit(result: ScanResult, fmt: str, fail_on: str | None, *,
          history: bool = False, verify: bool = False, report: str | None = None) -> None:
    """Render a ScanResult (table/json), write an optional report, enforce --fail-on."""
    scope = "git-history" if history else "filesystem"
    if report:
        out = write_report(result, report, scope)
        console.print(f"[green]Report written:[/] {out}")
    if fmt == "json":
        console.print_json(data=result_to_dict(result, scope))
    else:
        if result.findings:
...
@click.option("--report", default=None, help="Write a report file (.html/.json/.csv).")
...
def scan(path: str, entropy: bool, no_allowlist: bool, gitignore: bool,
         baseline_path: str | None, fmt: str, report: str | None, fail_on: str | None) -> None:
    ...
    _emit(scanner.scan(path), fmt, fail_on, history=False, report=report)
...
def history(path: str, entropy: bool, no_allowlist: bool, baseline_path: str | None,
            max_commits: int | None, fmt: str, report: str | None, fail_on: str | None) -> None:
    ...
    _emit(scanner.scan(path), fmt, fail_on, history=True, report=report)
...
def verify(path: str, entropy: bool, no_allowlist: bool, gitignore: bool,
           ...
           fmt: str, report: str | None, fail_on_valid: bool) -> None:
    ...
    if not items:
        ...
        _emit(result, fmt, None, verify=True, report=report)
        return
